...
void main() {
  test('does not treat generic internet access as portal online when user info is unreachable', () async {
  ...
  test('requires auth server confirmation before marking portal online', () async {
    final snapshot = await PortalStatusResolver.resolve(
      detectReality: () async => ('2', true, null),
      fetchUserInfo: () async => RadUserInfo(
        error: 'ok',
        onlineIp: '10.0.0.8',
      ),
    );
